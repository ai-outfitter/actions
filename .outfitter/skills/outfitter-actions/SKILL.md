...
description: Set up, review, or simplify agentic GitHub workflows built with ai-outfitter/actions. Use when adding GitHub-triggered agents, reducing duplicate agents or jobs, defining structured trigger context, or deciding how agents and skills should divide responsibility.

references:
  # SOURCE REPOSITORY: resolves inside the ai-outfitter/actions checkout or
...
Design agentic automation around a few stable agents, many focused skills,
...
3. Inspect the repository's `.agents` tree (settings, agents, skills) and
   GitHub workflows before editing.
4. Reuse a stable execution agent when its identity, policy, tools, and write
   boundaries fit the new behavior.
5. Put task-specific instructions in focused skills and keep the agent's
   system-prompt activation rules concise.
6. Prefer one reusable workflow for related triggers. Pass workflow-owned event
   metadata as `trigger_context` and let the agent activate the relevant
   skill.
...
another agent or near-duplicate Actions job. When the situation is close to
an existing skill, prefer adding a reference route inside that skill — or
appending a reference from the agent — over creating another skill.
